...
RESULTS = Path("/Users/chenyizhong/Documents/ISAAC_LAB_ROBOT_TRAINING/experiments/isaac/results")
SKIP = ("probe", "smoke")
...
def main() -> int:
    ...
    for d in sorted(RESULTS.iterdir()) if RESULTS.exists() else []:
        ...
        for r in csv.DictReader(open(ep)):
            ...
            if (r.get("condition") or r.get("method") or "").split("_")[0] in ("raw", "stop", "orca", "pydam"):
                continue
            try:
                rows.append({
                    "exp": d.name,
