...
只看部署情境（模型掛在護欄後面跑，訓練沒見過的種子）。欄位白話：**全隊完成時間** ＝
整局裡最後一隻狗走到終點所花的秒數（越短越好）；**護欄介入率** ＝ 每步裡護欄出手
修改指令的比例（越低＝模型自己越安全、越少靠護欄）；**違規步** ＝ 間距跌破 0.7 m
安全地板的步數；**最小間距** ＝ 全程最近的兩狗距離（越大越安全）。

**S2 探針（訓練沒見過的場景型態，2 狗對衝）—— 乾淨的勝利：**

| 條件 | 完成率 | 全隊完成時間 | 護欄介入率 | 違規步 | 最小間距 |
|---|---|---|---|---|---|
...
改良模型在擁擠的 S5 **完成率只剩 0.15、全隊完成時間爆到 52 s**（控制器是 0.95 / 11 s）。
