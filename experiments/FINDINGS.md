...
Terminology: **team completion time**（全隊完成時間）= seconds until the
LAST robot in the episode reaches its goal — NOT the per-robot average
(CSV column `makespan_s`; timeout counts as 60 s).
...
| E4.5 scale sweep | `run_e45_scale.py`: S5 × N {2,4,8,12,16} in the fixed ±6 m arena, 20 seeds | ✅ **ACCEPTED — liveness scales perfectly, safety degrades gracefully with density**: 100% completion and zero deadlock at EVERY N; team completion time sublinear (5.9→9.0 s); floor clean through N=8 (0.729, 15 viol steps), density erosion beyond (N=12 0.601, N=16 0.478 / 100 viol — F7 pressure, arena is fixed so density doubles). Filter p99 grows 0.10→0.70 ms — 3.5% of the 20 ms @ 50 Hz budget even at N=16 (pydam/SLSQP trend only; absolute robot numbers = E1.2 with dam+OSQP). ✔ dam numbers = pydam numbers (equivalence shown in E2, 1000 eps) |
...
- **F9 tangential linearization blind spot — γ is NOT a pure conservatism
  ...
  S3** (team completion time 10.3 vs 12.2 s, same floor). Guard improvement queued:
...
0.447); here it is 100% completion with wall-scraping transit (team completion time 18.2 s,
...
under-tracking):** team completion time +18–20 %; hard floors 0.05–0.17 m lower, gap
...
RQ5 問的是：**運行時收集到的失敗樣本，是不是完整、而且能拿來讓策略變好？**
本結論來自完全在 Isaac（真 Go2 四足 + 真 dam 過濾器）上跑的閉環實驗
...
> F14）三個要害：全在輕量模擬器、蒸餾的是手寫控制器而非策略本身、每池只有
> 30 局。本版取代之：改在 Isaac、糾正的是策略自己犯的錯、規模放大到百局級。
...
| 修正資料 | 360 | 199 637 | 534 | 1.000 | 過濾器介入 408 / 硬鬆弛 75 / 違規 51 |

欄位完整率全部 1.000（每筆事件的必要欄位都在）。值得記的一點：**修正資料
只用 60 種子，卻有 534 個事件，比原始資料 150 種子的 384 個還密**——因為初版
策略較弱、過濾器介入頻繁，策略犯錯多的地方修正紀錄就密，這正是「運行時失敗樣本」
該有的樣子。誠實註記：上下文視窗完整率 0.62–0.78（不是 1.0），因為很多違規事件
發生在接近終點、貼到該局結尾，前後 ±0.5 秒視窗被截斷——這是幾何必然，不是缺漏。

### 第二問「再利用價值」— 通過，但是雙面的（這才是論文故事）

只看部署情境（策略掛在過濾器後面跑，訓練沒見過的種子）。欄位白話：**全隊完成時間** ＝
整局裡最後一隻狗走到終點所花的秒數（越短越好）；**過濾器介入率** ＝ 每步裡過濾器出手
修改指令的比例（越低＝策略自己越安全、越少靠過濾器）；**違規步** ＝ 間距跌破 0.7 m
安全地板的步數；**最小間距** ＝ 全程最近的兩狗距離（越大越安全）。

**S2 探針（訓練沒見過的場景型態，2 狗對衝）—— 乾淨的勝利：**

| 條件 | 完成率 | 全隊完成時間 | 過濾器介入率 | 違規步 | 最小間距 |
|---|---|---|---|---|---|
| 控制器 + 過濾器 | 1.00 | 7.1 s | 0.281 | 1.3 | 0.71 |
| 初版策略 + 過濾器 | 1.00 | 7.1 s | 0.293 | 10.7 | 0.65 |
| **改良策略 + 過濾器** | 1.00 | 7.2 s | **0.179** | **0** | **0.92** |

改良策略：一樣快、完全完成、過濾器介入率降 36 %、零違規、間距更大。初版策略沒有
進步（0.293）——所以進步是修正資料帶來的，不是「訓練過就會變好」。

**S5（訓練場景的保留種子，6 狗巡航）—— 修正確實有效，但暴露代價：**

| 條件 | 完成率 | 全隊完成時間 | 過濾器介入率 | 違規步 | 最小間距 |
|---|---|---|---|---|---|
| 控制器 + 過濾器 | 0.95 | 11.0 s | 0.232 | 16.3 | 0.85 |
| 對照策略 + 過濾器 | 0.35 | 42.2 s | 0.109 | 16.1 | 0.85 |
| **改良策略 + 過濾器** | **0.15** | 52.3 s | **0.050** | **2.95** | **1.04** |

- **關鍵對照（證明是「過濾器修正」而非「資料變多」）**：改良策略過濾器介入率 0.050，
  對照策略 0.109（改良低一半）；違規步 2.95 對 16.1。對照策略吃了**同樣多**的資料，
  只是那些資料沒有過濾器修正，效果就差一截——排除了「只是資料比較多」這個解釋。
- **過濾器不可拿掉**：三個策略只要**關掉過濾器**，在 S5 一律崩潰（違規步 1000 以上、
  最小間距掉到 0.44）。單幀反應式策略學不會一個約束求解器 → 就算重訓過，運行時
  過濾器仍是必要的。這支持的是「架構」，不只是「資料」。

### 必須誠實並列的代價

改良策略在擁擠的 S5 **完成率只剩 0.15、全隊完成時間爆到 52 s**（控制器是 0.95 / 11 s）。
天真的單幀模仿把「安全」學成「過度保守」：它學會拉大間距（1.04 m）、少讓過濾器介入，
...
1. 修正資料可再利用：重訓後的改良策略需要過濾器的程度，明顯低於控制器、也低於
   吃等量資料的對照策略，且此結論在訓練沒見過的 S2 場景上零樣本成立。
2. 過濾器不可蒸餾：任何策略一關掉過濾器就在擁擠場景崩潰。
3. 天真重訓有代價：把安全學成過度保守，擁擠場景完成率崩到 0.15——資料有價值，但
   重訓配方要換更強的模仿方式（加入歷史、動作分塊，或只挑真正的修正樣本訓練）才能
   同時保住完成率。這是下一步。

**F15（發現）**：運行時安全指令過濾器的糾正動作，可作為訓練訊號再利用——把它回收重訓，
策略需要過濾器介入的頻率下降（S2 −36 %、S5 −78 %），且勝過吃等量普通資料的對照組，
證明價值來自「糾正」本身。但同一批資料也顯示兩件事：過濾器本身無法被重訓後的策略
取代（關掉即崩），且天真的單幀重訓會以完成率換安全（S5 完成率 0.95→0.15）。三者
共同支持論證：**運行時收集、離線重訓、過濾器持續掛著，並且要用更強的模仿配方**。

**F14（舊版，kinesim，已被 F15 取代但保留紀錄）**：輕量模擬器版本得到方向一致的
結論——過濾器糾正可作訓練訊號（天真 BC 重訓後介入率 −35…−65 %），但過濾器不可被重訓
策略取代。Isaac 版（F15）以真實四足、糾正策略本身、百局級規模與對照組坐實了它，
