...
class DifferentialInverseKinematicsAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        # print(f"\n[DiffIK] ========== process_actions ==========")
        # print(f"[DiffIK] raw_actions: {self._raw_actions[0].cpu().numpy()}")
        # print(f"[DiffIK] processed_actions (after scale={self._scale[0].cpu().numpy()}): {self._processed_actions[0].cpu().numpy()}")
        # print(f"[DiffIK] ee_pos_curr: {ee_pos_curr[0].cpu().numpy()}")
        # print(f"[DiffIK] ee_quat_curr: {ee_quat_curr[0].cpu().numpy()}")
